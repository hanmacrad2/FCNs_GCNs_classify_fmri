...
def check():
    print('Hey there Check')

    
def split_fmri_blocks_v2(self, fmri_idx, TR, block_duration, filter_flag):
        ...
        for t in np.arange(0,fmri_blocks.shape[0]):
            ...
            if filter_flag:
                #Filter #Normalise 
                fmri_block_t = nilearn.signal.clean(fmri_block_t, sessions= None, detrend=True, 
                                       standardize='zscore', confounds=None, low_pass= 0.1, 
                                       high_pass= 0.01, t_r = TR, ensure_finite=False)
            fmri_data.append(fmri_block_t)
